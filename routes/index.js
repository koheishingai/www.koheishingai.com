...
exports.stickup = function(req, res){
  res.render('stickup', { title: 'Express' });
};
...
exports.codegarages = function(req, res){
  res.render('codegarages', { title: 'Express' });
};
io.sockets.on('connection', function (socket) {
  ...
  socket.on('arinit', function(){
    ...
    for(var i = 0; i <  arnum; i++){
      socket.emit('arti', artitle[0][i+1], i, arin[0][i+1]);
    }
  ...
  socket.on('colinit', function(){
    ...
    require('col');
    for(var i = 0; i <  3; i++){
     socket.emit('colti', coltitle[0][i+1], i, colin[0][i+1]);
    }
    delete require.cache[require.resolve('col')]
  });
  socket.on('worksinit', function (data) {
    ...
    for(var i = 3; i <  6; i++){
      socket.emit('articleti', worksatitle[0][i+1]);
      console.log("doldol"+worksatitle[0][i+1]);
    }
