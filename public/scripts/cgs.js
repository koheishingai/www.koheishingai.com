$(function(){
  ...
  $('#full-page-link').click(function(){
    ...
    if(oneflg == 0 && elem == 0){
      ...
      if(hist == null){
        hist = {"0": 'http://www.koheishingai.com/codegarage'+'#'+rand};
        localStorage.setItem('history', JSON.stringify(hist));
      }else{
        var cnt = 0;
        for( i in JSON.parse(hist)){
          cnt++;
        }
        hist = JSON.parse(hist);
        hist[cnt] = 'http://www.koheishingai.com/codegarage'+'#'+rand;
        localStorage.setItem('history', JSON.stringify(hist));
      }
  ...
  socket.on('done', function () {
    if(elem == 1){
      $('#done').text('Save it.').fadeIn();
    }else{
      $('#done').text('Publish it.').fadeIn();
    }
  });
